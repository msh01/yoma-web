<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column v-permission="['admin','dept:edit','dept:del']" label="操作" width="130px" align="center"
                       fixed="right">
        <template slot-scope="scope">
          <udOperation
            :data="scope.row"
...
export default {
  ...
  cruds() {
    return CRUD({ title: '部门', url: '/core/baseDept', crudMethod: { ...crudDept } })
  },
  ...
  methods: {
    ...
    getDepts() {
      crudDept.getDepts({ enabled: true }).then(res => {
        ...
        this.depts = content.map(function(obj) {
          if (obj.hasChildren) {
            obj.children = null
          }
          return obj
    ...
    loadDepts({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        crudDept.getDepts({ enabled: true, pid: parentNode.id }).then(res => {
          ...
          let children = content.map(function(obj) {
            if (obj.hasChildren) {
              obj.children = null
            }
            return obj
